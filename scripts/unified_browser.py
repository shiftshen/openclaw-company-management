...
def _try_browser_init(acc: dict) -> dict:
    """对 unknown / login_required 的 profile 尝试浏览器初始化（打开真实页面检查登录态）"""
    ...
    positive_text = []
    if 'line' in platform:
        positive_text = ['Chats', 'Contacts', 'Analytics', 'Broadcasts']
    elif 'douyin' in platform:
        positive_text = ['工作台', '内容管理', '数据中心', '创作者服务中心']
    elif 'tiktok' in platform:
        positive_text = ['Upload', 'Creator Center', 'Analytics', 'LIVE']
    try:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # Check for negative login required terms
