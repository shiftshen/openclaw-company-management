...
1. **不要强制把远端改回 workspace-xmanx**：既然远端叫 `workspace-main`，请你优化本地 repo 中的所有脚本（包含 Python 脚本），将其中的硬编码 `/Users/shift/openclaw/workspace-xmanx` 改为自动检测机制（比如读取环境变量 `OPENCLAW_WORKSPACE`，若不存在则回退到当前约定，或用相对路径，但要求严谨稳健，不能影响线上）。
